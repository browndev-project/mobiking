...
class SubCategoryService {
  ...
  SubCategoryService({http.Client? client}) : _client = client ?? http.Client();
  ...
  bool _isCacheValid() {
    if (!_isHiveInitialized) return false;
  ...
  Future<void> clearAllBoxes() async {
    try {
      if (Hive.isBoxOpen(boxName) && _isHiveInitialized) {
        await _subCategoriesBox.clear();
        await _subCategoriesBox.close();
      }
      if (Hive.isBoxOpen('metadata') && _isHiveInitialized) {
