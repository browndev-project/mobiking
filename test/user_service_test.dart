  ...
  group('UserService', () {
    ...
    test('createUser posts and returns a UserModel', () async {
      final user = UserModel(
        ...
        cart: [CartModel(productId: 'p1', quantity: 2, price: 100.0, variantName: 'Default')],
      );

      when(mockDio.post('https://boxbudy.com/api/v1/users', data: anyNamed('data'))).thenAnswer(
        (_) async => Response(
          data: user.toJson(),
          statusCode: 201,
    ...
    test('getUserById returns a UserModel', () async {
      ...
      when(mockDio.get('https://boxbudy.com/api/v1/users/u1')).thenAnswer(
        (_) async => Response(
          data: userJson,
          statusCode: 200,
